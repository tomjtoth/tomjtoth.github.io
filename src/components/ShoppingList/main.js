class ShoppingList {
  ...
  static shuffle() {
    ...
    for (let i = 0; i < n && i < rec_len - 1; i++) {
      ...
      do {
        ...
        this.dish_indices.includes(j) ||
        this.recipies[j].tags.includes("dessert")
      );
  ...
  static rm_dish(dish_idx, splicing = true) {
    if (splicing) {
      this.dish_indices.splice(this.dish_indices.indexOf(dish_idx), 1);
      this.store("dishes", this.dish_indices);
    }
  ...
  static main(recipies_md) {
    this.recipies = Array.from(recipies_md.matchAll(this.re.dishes))
      .map((mo_dish) => {
        ...
        const tags = Array.from(
          (mo_dish.groups.tags ? mo_dish.groups.tags : "").matchAll(
            this.re.tags
          )
        ).map((mo_tag) => mo_tag[0]);
        ...
        const instructions = mo_dish.groups.descr.replaceAll(
          this.re.ingredients,
          "$1$2$3"
        );
        const ingredients = Array.from(
          mo_dish.groups.descr.matchAll(this.re.ingredients)
        );
    ...
    this.div_dishes.addEventListener("click", (ev) => {
      if (ev.target.innerText == "DEL") {
        if (
          confirm(
            `Poistetaanko ateriaa "${
              this.recipies[ev.target.parentNode.dish_idx].name
            }"?`
          )
        ) {
      ...
      // collapsible under this button getting opened
        ...
        const node =
            ev.target.tagName == "SPAN" ? ev.target.parentNode : ev.target,
  ...
  static store_item_states(reset = false) {
    localStorage.setItem(
      ...
      reset
        ? "[]"
        : JSON.stringify(
            Array.from(document.querySelectorAll("#items>button.active")).map(
              (btn) =>
                Array.prototype.indexOf.call(this.div_items.childNodes, btn)
            )
          )
    );
  }
