...
const runes = new Map(
  [
    ...
    return [rune, [sound, beginning, ending]];
  })
);
...
const queue = [];
let currently_playing_idx = 0;
let first_sound_counter = 0;
...
div_runes.addEventListener('click', ({ target }) => {
  ...
  if (first_sound_counter++ == 0) play_next_sound();
});
